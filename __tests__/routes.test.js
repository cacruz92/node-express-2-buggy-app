...
// TESTS BUG #1
  test("should return status code of 401 if authentication fails", async() => {
...
describe("Auth Middleware",  function () {
  test("should return 404 for unverified token", async function() {
...
describe("DELETE /users/[username]", function() {
  ...
  test("should return status code 404 if user not found", async function () {
    const response = await request(app).delete('/users/fakeuser123').send({ _token: tokens.u3 }); // u3 is admin
    expect(response.statusCode).toBe(404);
  })
